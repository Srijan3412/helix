    ...
    if (preGeneratedGraph) {
      ...
      actualLayers.forEach((layerName) => {
        ...
        mappedNodes.push({
          ...
          // Center the 220px header over the 170px file nodes by subtracting 25px
        ...
        // Add visual link from header to first node in column
      ...
      // Map File Nodes
      // Map File Nodes - Using the same layerMap
      (preGeneratedGraph?.nodes ?? []).forEach((node: any) => {
        const rawLayer = node.layer || "Services";
        const layerName = layerMap[rawLayer.toLowerCase()] || rawLayer;
        ...
        // Position nodes below header (start y=130, height=50, gap=40)
    ...
    tourTimerRef.current = setInterval(() => {
      setTourIdx(prev => {
        ...
        if (reactFlowInstance) {
          // Try to find layer header first
