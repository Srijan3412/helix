...
export default function LanguageBreakdown({ languages, totalLines: customTotalLines, entryPoints }: LanguageBreakdownProps) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 text-left">
      ...
      >
        <h3 className="dash-section-heading text-emerald-400 uppercase tracking-[0.06em] mb-4 flex items-center gap-2">
          <Code size={16} />
          Language Breakdown
        </h3>
              ...
              >
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="dash-body text-white font-medium">{language}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="dash-metadata text-muted-foreground">{lines.toLocaleString()} lines</span>
                    <span className={`dash-metadata font-semibold ${colors.text}`}>{percentage.toFixed(1)}%</span>
                  </div>
                </div>
        ...
        <div className="mt-6 pt-4 border-t border-border/50 flex items-center justify-between">
          <span className="dash-metadata text-muted-foreground font-medium">Total Lines of Code</span>
          <span className="dash-card-title text-white">{totalLines.toLocaleString()}</span>
        </div>
      </motion.div>
      ...
      >
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <span className="dash-filepath text-zinc-300 truncate max-w-xs">{entryPath}</span>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <span className="dash-metadata text-muted-foreground w-8 text-right font-medium">
        ...
        <p className="mt-4 dash-metadata text-muted-foreground flex items-center gap-1">
